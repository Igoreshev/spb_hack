...
4. Для поиска аномалий используется `IsolationForest`.

5. Финальный anomaly score получается из скора `IsolationForest` с последующей нормализацией.
...
├── best_model.pkl       # основная версия IsolationForest для инференса
